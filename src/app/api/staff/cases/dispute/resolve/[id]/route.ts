...
/** `id` here is the Dispute's own `_id`, not the case_id — see CaseController#resolveDispute. */
export async function POST(_request: Request, { params }: RouteParams) {
  try {
    const session = await requireStaffRole(CASE_ACTION_ROLES);
    const { id } = await params;

    const dispute = await staffBackendRequest<Dispute>(session.token, `/cases/dispute/resolve/${id}`, {
      method: "POST",
    });

    return jsonSuccess(dispute, "Dispute resolved successfully.");
  } catch (error) {
