...
/** Privileged view — mirrors `CallController#findCallList` (`GET /calls/list`). */
export async function GET(request: NextRequest) {
  try {
    const session = await requireStaffRole(CALL_PRIVILEGED_ROLES);
    const query = request.nextUrl.searchParams.toString();
    const result = await staffBackendRequest<ListResult<Call>>(session.token, `/calls/list?${query}`);
